var initFormsLookup = (function () {
	...
	*/
	...
	function _templateObject2$2() {
	  var data = _taggedTemplateLiteral(["<div class=\"jcc-forms-filter__results-header\">Found ", " forms matching \"", "\"</div>"]);
	...
	var FormResults = function FormResults(actions, _ref) {
	  ...
	  var ResultsHeader = function ResultsHeader() {
	    return browser(_templateObject2$2(), response.length, query);
	  };

	  var maxResultsOnFirstLoad = 40;

	  if (response.length > maxResultsOnFirstLoad && !showMoreForms) {
	    return browser(_templateObject3$1(), ResultsHeader(), response.slice(0, maxResultsOnFirstLoad).map(FormResult), function () {
	      return actions.onShowMoreFormsClick();
	    }, response.length - maxResultsOnFirstLoad);
	  } else if (response.length > 0) {
	    return browser(_templateObject4(), ResultsHeader(), response.map(FormResult));
	  } else {
	...
	var SearchResults = function SearchResults(actions, _ref) {
	  ...
	  if (!response) {
	    ...
	    return browser(_templateObject3$2(), CategoryAlert(actions, {
	      query: query
	    }), FormResults(actions, {
	      query: query,
	      response: response,
	...
	function initFormsLookup(containerEl) {
	  ...
	  containerEl.appendChild(browser(_templateObject$4(), allFormsPageUrl));
	  ...
	  var render = function render() {
	    ...
	    resultsContainer.appendChild(SearchResults(actions, state)); // Render mobile
